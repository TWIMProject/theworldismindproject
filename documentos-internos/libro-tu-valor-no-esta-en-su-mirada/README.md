...
| **14 may 2026** | Daniel cierra las 2 decisiones editoriales abiertas (cross-sell programa · refuerzo B2B): **NO cross-sell explícito** · **SÍ refuerzo con 2-3 escenas adicionales en contextos variados** (laboral, amistad, familia, además de pareja). Y decide subir portada/contraportada como PDF a Amazon KDP en lugar de Cover Creator. | `estado-completitud-2026-05-14.md` §3.2 actualizado con las 2 decisiones cerradas y propuesta operativa de 5 capítulos candidatos para las escenas. **Generación v1 de portada/contraportada** en `portada/` con dos PDFs: `portada-front-preview.pdf` (solo portada delantera 6×9" para mockups) + `portada-wraparound-kdp.pdf` (wraparound completo back+spine+front listo para subir a Amazon KDP). Script reproducible en `scripts/generar-portada-libro-tu-valor.py` con lomo calculado para 250 páginas provisional. Limitación documentada: fuentes built-in Times-Roman + Helvetica como aproximación de Instrument Serif + Barlow Condensed (versión final pre-imprenta requiere sustituir por las TTF correctas). |
| **14 may 2026** (tarde) | Daniel descarta la portada v1 (verde TWIM dominante) por no cumplir el criterio dopamina-comercial. Code propone 3 conceptos v2 (A·ojo · B·cuerda · C·llave). Daniel elige Concepto C «La llave dentro». Brief de maquetación interior Word→KDP creado. Regla inviolable dopamina-comercial añadida a CLAUDE.md. Daniel también pide a Code redactar las 3 escenas adicionales. | Portada v2 generada con paleta crema + verde botella + dorado + placeholder de la llave. `brief-diseno-portada-v2-concepto-c.md` con dirección artística completa para diseñador/IA generativa. `maquetacion/brief-word-kdp.md` con 570 líneas para que Daniel maquete el interior. `escenas-adicionales-2026-05-14.md` con las 3 escenas redactadas (laboral · amistad · familia) en voz Daniel listas para pegar al `.docx`. |
| **15 may 2026** | Daniel pide a Code generar e integrar la llave vectorial directamente en la portada (sin esperar a diseñador externo). Acto seguido, retirar el placeholder ISBN/EAN-13 porque Amazon KDP lo añade en su flujo. | **Llave vectorial procedural integrada** en `draw_key()` del script · primitivas reportlab (círculos · rectángulos · bezier para el cordel) · trazo verde botella `#1F4A3A` + relleno dorado mate `#C19E5A` + cordel beige cálido. ISBN/EAN-13 eliminado de `draw_back()`. PDFs regenerados (`portada-front-preview.pdf` 41.6 KB · `portada-wraparound-kdp.pdf` 82 KB). Daniel aprueba visualmente. Documento de cierre de sesión generado en `documentos-internos/cierre-sesion-2026-05-15.md` para orientación de la próxima sesión. |
