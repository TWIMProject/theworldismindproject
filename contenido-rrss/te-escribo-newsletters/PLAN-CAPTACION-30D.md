...
- ✅ **Carta Promo Directo 8 jun «La voz que te juzga»** programada vía MCP MailerLite (ID `187974522939377362`) para miércoles **3 junio 2026 19:00 CEST** (status `ready`, `scheduled_for: 2026-06-03 17:00 UTC` = 19:00 CEST). Audiencia: Newsletter Home + Reto + Lead Magnet Dependencia + Lista General (49 destinatarios). HTML conservado en `contenido-rrss/te-escribo-newsletters/carta-promo-directo-8-jun-voz-que-te-juzga.html`. Copy verbatim del §5.1 del doc del directo. **Pendiente manual Daniel:** cambiar idioma a Español en panel (Configuración del email → idioma) — la campaña nace en `language_id: 4` (en-US) por arista del conector (`directo-...md` §9.2 arista #1); si no se cambia antes del envío, el bloque legal inyectado por MailerLite saldría en inglés.
...
1. ~~**Campaña promo `187974522939377362` en MailerLite** · status `ready` (programada para 3-jun 19:00 CEST) bloquea edición vía API.~~ ✅ **RESUELTO vía API el 25-may 06:49 UTC** · `cancel_campaign` → `update_campaign` (name `Te escribo · Promo Directo 7 jun · La voz que te juzga`, subject `Te invito a una hora, en directo, sobre la voz que te juzga`, HTML actualizado con «Domingo 7 de junio, 19:00» + URL correcta) → `schedule_campaign` para `2026-06-03 19:00 CEST` (= 17:00 UTC). Status final `ready`, `scheduled_for: 2026-06-03 17:00:00`. **Solo queda manual Daniel** · cambiar `language_id` a Español en panel (Configuración del email → idioma) — arista conocida del MCP (no expone language_id en `update_campaign`); si no se hace antes del envío, el bloque legal inyectado por MailerLite saldría en inglés.
2. ~~**Google Calendar** · actualizar evento del directo a domingo 7 jun 19:00 (link Meet `https://meet.google.com/fsu-scrz-res` se mantiene).~~ ✅ **RESUELTO vía MCP Google Calendar el 25-may** · evento NO existía previamente (solo aparecían seminarios recurrentes), CREADO desde cero · ID `p5f6uqf6fsr0sitslqrciq7hu8`, título «Directo · La voz que te juzga (TWIM Project)», `2026-06-07T19:00:00 → 20:00 Europe/Madrid`, color 10 (Basil verde), descripción completa con enlace Meet existente + landing + nota corrección, 3 recordatorios popup (24h + 1h + 15min antes).
...
|---|---|---|---|---|---|---|---|
| Instagram | 861 | 343 | 8 | 4 | 0 | 0 | 14 |
| LinkedIn | 13 (impresiones) | 9 | 1 (reacción) | 0 | 0 | 0 | 1 |
| Facebook | ~0 | — | — | — | — | — | — |
| X | 0 (recién subido) | — | — | — | — | — | — |
...
- **Saves+shares a ~17 h = 0.** El **umbral declarado a 7 días para confirmar la hipótesis del packaging es ≥11-16** (BRIEFING §6.7 + métricas #2 §8). Arranque muy por debajo; quedan ~6 días reales hasta cierre de ventana, pero la señal temprana es débil.
- **Comentarios — desglose honesto (corregido 20 may por Daniel):** los 4 comentarios brutos se descomponen en **2 respuestas del propio Daniel + 1 amigo (`_pointofdrew`) + 1 sales-grabber externo (`heal_.unity`)**. **Engagement orgánico externo cualificado a ~17 h ≈ 0**, mismo nivel que el #2. La lectura inicial «4 comentarios cualitativos» era optimista; el #3 **no está mostrando señal diferencial sobre el #2** todavía. Queda margen hasta los 7 días reales.
- **No repostear** al sales-grabber `heal_.unity` por DM sin auditar la cuenta.
